...
class Bot extends Game {
    constructor(n, m) {
        super();
        this.memoizedComp = this.memoizeComp(this.computeMove).bind(this);
        this.memoizedAlpha = this.memoizeAlphaSquare(this.alphabeta).bind(this);
        this.memoizedSquare = this.memoizeAlphaSquare(this.isSquare).bind(this);
        this.currGameString = this.createGameString(n, m);
        this.n = n;
        this.m = m;
    }
    createGameString(n, m) {
        let string = "";
        for (let i = 0; i < m; i++) {
            string += (n - 1).toString();
        }
    ...
    movesInOne(gameString) {
        if (gameString === null || gameString === undefined)
            return null;
    ...
    isSquare(gameString) {
        ...
        if (gameString === (null || undefined || "0"))
            return false;
        // console.log(`isSquare passed the valid`)
        let height = gameString[0];
        for (let j = 0; j < gameString.length; j++)
            if (gameString[j] != height)
                return false;
        return parseInt(height) + 1 === gameString.length;
    }
    memoizeComp(func) {
        let cache = {};
        const memoized = (gameString, i, j) => {
            let key = gameString + `[${i}][${j}]`;
    ...
    memoizeAlphaSquare(func) {
        let cache = {};
        const memoized = (gameString) => {
    ...
    win(gameString) {
        ...
        if (gameString === (null || undefined || "0"))
            ...
            return this.computeMove(gameString, 1, 1);
    ...
    alphabeta(gameString) {
        ...
        if (this.memoizedSquare(gameString))
            return true;
        // if(this.memoizedSquare(gameString)) return true;
        let moves = this.movesInOne(gameString);
        // console.log(`moves is: ${moves}`)
        for (let move of moves) {
            if (!this.memoizedAlpha(move))
                return true;
        }
        return false;
    }
    updateGameString(i, j) {
        ...
        for (let index = 0; index < a.length; index++) {
            for (let j = 0; j < a[index].length; j++) {
                if (j >= this.currGameString.length) {
                    ...
                    if (this.n - index - 1 > parseInt(this.currGameString[j]))
                        this.globalGameState.array[index][j] = false;
    ...
    updateShapesByString() {
        ...
        for (const circle of this.globalGameState.shapes) {
            if (parseInt(this.currGameString[circle.j]) <= circle.i)
...
// }
let replaceAt = function (og, index, replacement) {
...
if ((n && m) != (null && undefined)) {
    ...
    canvas.addEventListener("click", (e) => {
        ...
        if (circlePos[0] != -1) {
            ...
            let winString = bot.win(bot.currGameString);
